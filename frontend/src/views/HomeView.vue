...
const kargatuDatuak = async () => {
  try {
    ...
    const [resUser, resProd, resHitz] = await Promise.all([
      ...
      axios.get('hitzorduak', { headers: { Authorization: `Bearer ${token}` } }).catch(() => ({ data: [] }))
    ])
...
<template>
  <div v-if="!loading" class="space-y-12 animate-fade-in pb-20">
    
    <header class="bg-indigo-900 text-white p-16 rounded-[4rem] shadow-[0_35px_60px_-15px_rgba(79,70,229,0.4)] border-b-[12px] border-indigo-400 relative overflow-hidden">
      <div class="relative z-10">
        ...
        <p class="mt-8 text-indigo-100/70 max-w-2xl text-2xl font-medium italic leading-relaxed">
          {{ user.rola === 'irakasle' 
      ...
      <div class="absolute right-[-2rem] top-[-2rem] text-[20rem] opacity-10 font-black italic select-none leading-none">
        KAIXO
      </div>
    </header>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-12 pt-6">
      ...
      <router-link to="/stock" class="group bg-white p-16 rounded-[4.5rem] shadow-2xl hover:shadow-indigo-200/50 transition-all relative overflow-hidden border border-indigo-100">
        <span class="text-7xl mb-8 block">🧴</span>
        <h3 class="text-6xl font-black uppercase italic text-indigo-950 group-hover:translate-x-6 transition-transform leading-none">Inbentarioa</h3>
        <p class="text-indigo-600 mt-6 font-bold uppercase tracking-widest text-sm italic">Materialaren kontrol osoa</p>
        <div class="absolute right-[-2rem] bottom-[-2rem] text-[15rem] opacity-[0.03] font-black italic select-none">STOCKA</div>
      </router-link>

      <router-link v-if="user.rola === 'irakasle'" to="/admin/ikasleak" class="group bg-slate-900 p-16 rounded-[4.5rem] shadow-2xl hover:scale-[1.02] transition-all text-white relative overflow-hidden">
        <span class="text-7xl mb-8 block">📊</span>
        <h3 class="text-6xl font-black uppercase italic group-hover:translate-x-6 transition-transform leading-none">Jarraipena</h3>
        <p class="text-indigo-400 mt-6 font-bold uppercase tracking-widest text-sm italic">Ikasleen log-ak kudeatu</p>
        <div class="absolute right-[-2rem] bottom-[-2rem] text-[15rem] opacity-10 font-black italic select-none">LOGAK</div>
      </router-link>
      
      <router-link v-else to="/hitzorduak" class="group bg-violet-600 p-16 rounded-[4.5rem] shadow-2xl hover:scale-[1.02] transition-all text-white relative overflow-hidden">
        <span class="text-7xl mb-8 block">📅</span>
        <h3 class="text-6xl font-black uppercase italic group-hover:translate-x-6 transition-transform leading-none">Hitzorduak</h3>
        <p class="text-violet-100 mt-6 font-bold uppercase tracking-widest text-sm italic">Agenda eta txandak kudeatu</p>
        <div class="absolute right-[-2rem] bottom-[-2rem] text-[12rem] opacity-10 font-black italic select-none">HITZORDUAK</div>
      </router-link>
